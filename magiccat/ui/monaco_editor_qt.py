...
function __sendBridge(method, args) {
  ...
  } else if (__bridge && __bridge[method]) {
    __bridge[method].apply(__bridge, args || []);
  }
}
function __emitSelectionState() {
  if (__editor) {
    ...
    if (window.chrome && window.chrome.webview) {
      __sendBridge('emitSelectionChanged', [selected, text]);
    } else if (__bridge) {
...
if (!(window.chrome && window.chrome.webview)) {
  ...
  __channelScript.onload = function () {
...
require(['vs/editor/editor.main', 'vs/basic-languages/sql/sql'], function () {
  ...
  __editor.onDidChangeModelContent(function () {
    if (window.chrome && window.chrome.webview) {
      __sendBridge('emitChanged');
    } else if (__bridge) {
      __bridge.emitChanged();
    }
  });
