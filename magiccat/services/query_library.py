...
- 查询内容：`<MAGICCAT_HOME>/queries/<profile_id>/<schema>/<name>.sql`。
- 元数据（名称/所属 schema/更新时间）：SQLite favorites（kind='query'）。
...
class QueryLibrary:
    ...
    def save(self, profile_id: str, name: str, content: str, schema: str = "") -> None:
        self._store.save(profile_id, name, content, schema=schema)
