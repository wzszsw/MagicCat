...
- 主窗口显示前先预热一个**不加入标签栏**的 Monaco `QueryWorkspace`；`MainWindow` 保持隐藏，
  启动期间显示独立的 `StartupSplash`，等 `MonacoEditorWidget.readyChanged`（页面成功初始化
  或明确失败）后才显示主窗口并关闭 splash，避免首个 `QWebEngineView` 的 Chromium 初始化
  白闪。首次“新建查询”直接复用该实例，不在窗口显示后继续创建隐藏预热实例。
  `MonacoEditorWidget.load()` 必须幂等；启动时中央仍只显示固定“对象”页。
...
- **对话框提交时机**：可预知的输入校验、名称冲突、路径映射和本地配置/任务落盘必须在
  `QDialog.accept()` 前完成；提交失败时保持对话框打开并在当前对话框内提示，禁止先关闭
