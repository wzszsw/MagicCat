...
- **当前连接 = 树跟手（重要，对标 Navicat）**：Navicat **全局无「当前连接选择控件」**（顶栏功能切换器不含连接下拉）。
  ...
  - **每查询标签独立记忆连接/库（影响不扩散）**：每个查询编辑标签用 `_conn_profile_id`/`_conn_schema` 记忆自己的连接/库；
    切换标签时头部条加载该标签记忆的连接/库（`_apply_editor_conn`），在头部条改连接/库则写回当前标签（`_write_conn_to_editor`）。
    树激活/手动切换只作用于**当前激活的查询标签**，不影响其它标签。
