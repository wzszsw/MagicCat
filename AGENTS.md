...
- 调试构建可使用 PyInstaller `--console` 以便查看异常栈；正式发行构建必须使用 `--windowed`，Windows 统一通过 `scripts/build_windows.ps1` 重建 bridge/应用、自检、便携包和 Inno 安装器，macOS arm64 通过 `scripts/build_macos.sh` 重建 bridge/jlink/应用并生成 DMG，避免复用旧 `dist` 产物。
